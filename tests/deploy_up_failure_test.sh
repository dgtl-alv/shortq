...
case "$*" in
  ...
  *" config --quiet") exit 0 ;;
  *" up -d --build --remove-orphans") exit 42 ;;
  *)
...
output="$({
  ...
  DEPLOY_TEST_CURL_MARKER="$tmp/curl-called" \
  PATH="$fake_bin:$PATH" \
  bash "$subject"
} 2>&1)"
status=$?
...
if [[ "$status" -eq 0 ]]; then
  printf 'FAIL: deploy succeeded after compose up failed\n%s\n' "$output" >&2
  exit 1
fi

if [[ -e "$tmp/curl-called" ]]; then
  printf 'FAIL: health check ran after compose up failed\n%s\n' "$output" >&2
  exit 1
fi
...
printf 'PASS: compose up failure stops health checks and preserves rollback state\n'
